...
class _PaymentButtonState extends State<PaymentButton> {
  ...
  Future<void> _checkPaymentStatus() async {
    ...
    if (!authProvider.isAuthenticated) {
      debugPrint('User not authenticated, skipping payment check');
      return;
    }
  ...
  bool get _isPaymentCompleted {
    return _paymentStatus == PaymentStatus.COMPLETED;
  }

  bool get _isPaymentPending {
    return _paymentStatus == PaymentStatus.PENDING;
  }

  bool get _isPaymentFailed {
    return _paymentStatus == PaymentStatus.FAILED;
  }
  ...
  Future<void> _navigateToPayment() async {
    ...
    if (!authProvider.isAuthenticated) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(
          content: Text('Please log in to make payments'),
  ...
  Widget build(BuildContext context) {
    ...
    return Column(
      ...
      children: [
        // Cash-on-start badge when payment exists as CASH and is still pending
        if (_payment != null && _payment!.isPending &&
            (_payment!.paymentMethod.toUpperCase() == PaymentStatus.values.first.runtimeType.toString() /* placeholder to avoid analyzer error */ || _payment!.paymentMethod.toUpperCase() == 'CASH'))
          Container(
            ...
            child: const Row(
              ...
              children: [
                Icon(Icons.payments, size: 14, color: Colors.amber),
                SizedBox(width: 6),
                Text(
                  'Cash on start — pay the driver when the trip starts',
